...
public class WebSocketReader extends Thread {
    ...
    public WebSocketReader(Handler master, Socket socket, WebSocketOptions options, String threadName) {
        ...
        this.mBuffer = ByteBuffer.allocateDirect(options.getMaxFramePayloadSize() + 14);
    ...
    private boolean processData() throws Exception {
        ...
        if (mFrameHeader == null) {
            ...
            if (mBuffer.position() >= 2) {
                ...
                if (mBuffer.position() >= header_len) {
                    ...
                    if (payload_len1 == 126) {
                        ...
                        payload_len = ((long) (0xff & mBuffer.get(i + 0)) << 56) |
                                ((long) (0xff & mBuffer.get(i + 1)) << 48) |
                                ((long) (0xff & mBuffer.get(i + 2)) << 40) |
                                ((long) (0xff & mBuffer.get(i + 3)) << 32) |
                                ((long) (0xff & mBuffer.get(i + 4)) << 24) |
                                ((long) (0xff & mBuffer.get(i + 5)) << 16) |
                                ((long) (0xff & mBuffer.get(i + 6)) << 8) |
                                ((long) (0xff & mBuffer.get(i + 7)));
                    ...
                    if (masked) {
                        ...
                        for (int j = 0; j < 4; ++j) {
                            mask[i] = (byte) (0xff & mBuffer.get(i + j));
                        }
                    ...
                    return mFrameHeader.getPayloadLength() == 0 || mBuffer.position() >= mFrameHeader.getTotalLength();
            ...
            if (mBuffer.position() >= mFrameHeader.getTotalLength()) {
                ...
                if (mFrameHeader.getPayloadLength() > 0) {
                    ...
                    mBuffer.position(mFrameHeader.getHeaderLength());
                    mBuffer.get(framePayload, 0, (int) mFrameHeader.getPayloadLength());
                }
                mBuffer.position(mFrameHeader.getTotalLength());
    ...
    public void run() {
        ...
        try {
            while (mState != ReaderState.STATE_CLOSED) {
                int bytesRead = inputStream.read(mNetworkBuffer);
                if (bytesRead > 0) {
                    mBuffer.put(mNetworkBuffer, 0, bytesRead);
                    while (consumeData()) {
                    }
                } else if (mState == ReaderState.STATE_CLOSED) {
                    notify(new WebSocketMessage.Close(WebSocketCloseCode.NORMAL)); // Connection has been closed normally
                    Log.d(TAG, "run() : Close normal");
                } else if (bytesRead < 0) {
