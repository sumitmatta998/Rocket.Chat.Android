...
public class WebSocketWriter extends Thread {
    ...
    public WebSocketWriter(Handler master, Socket socket, WebSocketOptions options, String threadName) {
        ...
        this.mBuffer = ByteBuffer.allocate(options.getMaxFramePayloadSize() + 14);
    ...
    private void sendClientHandshake(WebSocketMessage.ClientHandshake message) throws IOException {
        ...
        mBuffer.put(("GET " + path + " HTTP/1.1" + CRLF).getBytes());
        mBuffer.put(("Host: " + message.getURI().getHost() + CRLF).getBytes());
        mBuffer.put(("Upgrade: WebSocket" + CRLF).getBytes());
        mBuffer.put(("Connection: Upgrade" + CRLF).getBytes());
        mBuffer.put(("Sec-WebSocket-Key: " + newHandshakeKey() + CRLF).getBytes());

        if (message.getOrigin() != null) {
            mBuffer.put(("Origin: " + message.getOrigin().toString() + CRLF).getBytes());
        }

        if (message.getSubprotocols() != null && message.getSubprotocols().length > 0) {
            mBuffer.put(("Sec-WebSocket-Protocol: ").getBytes());
            for (int i = 0; i < message.getSubprotocols().length; ++i) {
                mBuffer.put((message.getSubprotocols()[i]).getBytes());
                mBuffer.put((", ").getBytes());
            }
            mBuffer.put((CRLF).getBytes());
        }

        mBuffer.put(("Sec-WebSocket-Version: " + WEB_SOCKETS_VERSION + CRLF).getBytes());
        mBuffer.put((CRLF).getBytes());
    }
    ...
    protected void sendFrame(int opcode, boolean fin, byte[] payload, int offset, int length) throws IOException {
        ...
        if (len <= 125) {
            ...
            b1 |= (byte) (126 & 0xff);
            mBuffer.put(b1);
            mBuffer.put(new byte[]{(byte) ((len >> 8) & 0xff), (byte) (len & 0xff)});
        } else {
            b1 |= (byte) (127 & 0xff);
            mBuffer.put(b1);
            mBuffer.put(new byte[]{(byte) ((len >> 56) & 0xff),
                    (byte) ((len >> 48) & 0xff),
                    (byte) ((len >> 40) & 0xff),
                    (byte) ((len >> 32) & 0xff),
                    (byte) ((len >> 24) & 0xff),
        ...
        if (len > 0) {
            ...
            mBuffer.put(payload, offset, length);
        }
    }
    ...
    protected void processAppMessage(Object msg) throws WebSocketException, IOException {
        throw new WebSocketException("unknown message received by WebSocketWriter msg" + msg);
    }
    ...
    public void writeMessageToBuffer(Message message) {
        try {
            ...
            mOutputStream.write(mBuffer.array(), mBuffer.position(), mBuffer.limit());
            mOutputStream.flush();
